...
pic_prefix = "_"+str(hyper_para["epochs"]) +"_"+str(hyper_para["M"]) + "_" +str(hyper_para["N"]) +"_"+str(hyper_para["batch_size"])+str(time_start)
...
for rep in range(3):
  ...
  if not (os.path.exists('./models/model_short_rep' + str(rep))):
    ...
    for epoch in range(hyper_para['epochs']):
      ...
      loss_sum = 0
      # 200 for validation
      for _, (inputs, labels) in enumerate(zip(short_arrays_sam[-200:], short_labels_sam[-200:])):
        torch.cuda.empty_cache()
        prediction = model_short(inputs).squeeze()
        loss = loss_function(prediction, labels)
        loss_sum += loss.item()
  ...
  if not (os.path.exists('./models/model_long_rep' + str(rep))):
    ...
    for epoch in range(hyper_para['epochs']):
      ...
      loss_sum = 0
      # 200 for validation
      for _, (inputs, labels) in enumerate(zip(long_arrays_sam[-200:], long_labels_sam[-200:])):
        torch.cuda.empty_cache()
        prediction = model_long(inputs).squeeze()
        loss = loss_function(prediction, labels)
        loss_sum += loss.item()
